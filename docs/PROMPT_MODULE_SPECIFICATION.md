...
プロンプトモジュールは、AIへの指示を構造化し、再利用可能なモジュールとして管理するTypeScriptフレームワークである。
...
- **完全性**：モジュールはcreateContextで必要なデータをすべて定義
...
実際の動作するアプリケーションとして、`@moduler-prompt/simple-chat`パッケージを提供する。

### Simple Chat - リファレンス実装

[packages/simple-chat](../packages/simple-chat)は、Moduler Promptフレームワークを使用したチャットアプリケーションのサンプル実装である。
...
プロンプトモジュールフレームワークは、AIプロンプトの構造化と管理を体系的に行うためのソリューションである。3つの大セクション（Instructions/Data/Output）による明確な責務分離と、プロンプトインジェクション対策により、安全で保守性の高いプロンプト管理を実現する。